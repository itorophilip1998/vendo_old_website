...
class Commissions
{
    ...
    public function getUsersInPool($career_level)
    {
        ...
        $sql = "SELECT u.id, m.active_direct_count, m.month_ago FROM User u JOIN (SELECT COUNT(*) as active_direct_count, upline_user_id, (DATEDIFF(NOW(), GREATEST(date_of_entry, '2020-08-01')) DIV 30 + 1) as month_ago FROM User WHERE broker_registration_complete = 1 AND balance >= :balance GROUP BY upline_user_id, month_ago) m ON u.id = m.upline_user_id WHERE max_career_level = :level ORDER BY u.id, m.month_ago";
        $sth = $pdo->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
