...
try {
	...
	$sumCompleteOpeningTransactionsThisMonth = Bitcoin::getTotalOpenBuyingTransactionsSumInUsdForMonth(date('Y-m'));
	$sumCompleteOpeningTransactionsLastMonth = Bitcoin::getTotalOpenBuyingTransactionsSumInUsdForMonth(date('Y-m', mktime(0, 0, 0, date('m')-1, 1, date('Y'))));
	$sumCompleteOpeningTransactionsSecondLastMonth = Bitcoin::getTotalOpenBuyingTransactionsSumInUsdForMonth(date('Y-m', mktime(0, 0, 0, date('m')-2, 1, date('Y'))));
	...
	<div id="divToBlur">
		...
		<div class="main-color">
			...
			<div class="col <?php if (count($pendingTransactions) == 0) echo "hidden"; ?>" id="table-lots">
				...
				<!-- completed memberships overview -->
				</br>
				<h2>
					<?php echo localize("bo_pending_bitcoins_sum_memberships_title") ?>
				</h2>
				<table id="table-totalsum-complete-memberships" class="table table-sm mt-2 table-borderless align-self-center">
					<thead class="bo_text_header table_bottom_line">
						<tr>
							<th><?php echo localize("bo_vq_summary_month") ?></th>
							<th><?php echo localize("bo_pending_bitcoins_sum_new_memberships") ?></th>
							<th><?php echo localize("bo_vq_summary_sum") ?></th>
						<tr>
					</thead>
					<tbody class="bo_text_table_body">
						<tr>
							<td><?php echo date('F Y'); ?></td>
							<td><?php echo (is_null($sumCompleteOpeningTransactionsThisMonth['total'])) ? "0" : $sumCompleteOpeningTransactionsThisMonth['total']; ?></td>
							<td><?php echo (is_null($sumCompleteOpeningTransactionsThisMonth['totalsum'])) ? "0" : $sumCompleteOpeningTransactionsThisMonth['totalsum']; ?> USD</td>
						</tr>
						<tr>
							<td><?php echo date('F Y', mktime(0, 0, 0, date('m')-1, 1, date('Y'))); ?></td>
							<td><?php echo (is_null($sumCompleteOpeningTransactionsLastMonth['total'])) ? "0" : $sumCompleteOpeningTransactionsLastMonth['total']; ?></td>
							<td><?php echo (is_null($sumCompleteOpeningTransactionsLastMonth['totalsum'])) ? "0" : $sumCompleteOpeningTransactionsLastMonth['totalsum']; ?> USD</td>
						</tr>
						<tr>
							<td><?php echo date('F Y', mktime(0, 0, 0, date('m')-2, 1, date('Y'))); ?></td>
							<td><?php echo (is_null($sumCompleteOpeningTransactionsSecondLastMonth['total'])) ? "0" : $sumCompleteOpeningTransactionsSecondLastMonth['total']; ?></td>
							<td><?php echo (is_null($sumCompleteOpeningTransactionsSecondLastMonth['totalsum'])) ? "0" : $sumCompleteOpeningTransactionsSecondLastMonth['totalsum']; ?> USD</td>
						</tr>
					</tbody>
				</table>
			</div>
